...
export async function registerRoutes(app: Express): Promise<void> {
  ...
  app.get("/api/system/overview", async (req, res) => {
    try {
      ...
      const stats = {
        ...
        totalValue: contracts.reduce((sum, c) => sum + (c.giaTriHopDong || 0), 0),
        totalPayments: payments.length,
        pendingPayments: payments.filter(p => p.trangThai === "Chưa thanh toán").length,
        completedPayments: payments.filter(p => p.trangThai === "Đã thanh toán").length,
        totalEquipment: equipment.length,
        totalDocuments: documents.length,
        totalProgressSteps: progressSteps.length,
        inProgressSteps: progressSteps.filter(p => p.trangThai === "Đang thực hiện").length,
        completedSteps: progressSteps.filter(p => p.trangThai === "Hoàn thành").length,
        pendingSteps: progressSteps.filter(p => p.trangThai === "Chưa bắt đầu").length
      };
  ...
  app.get("/api/download/project", (req, res) => {
    const filePath = "/home/runner/workspace/vietnamese-contract-management.tar.gz";
    res.download(filePath, "vietnamese-contract-management.tar.gz", (err) => {
      if (err) {
  ...
  app.get("/api/dashboard/charts", async (req, res) => {
    try {
      ...
      const paymentStatusData = [
        { name: "Đã thanh toán", value: payments.filter(p => p.trangThai === "Đã thanh toán").length },
        { name: "Chưa thanh toán", value: payments.filter(p => p.trangThai === "Chưa thanh toán").length || payments.length }
      ];

      const progressSteps = await db.select().from(schema.buocThucHien);
      const progressStatusData = [
        { name: "Hoàn thành", value: progressSteps.filter(p => p.trangThai === "Hoàn thành").length },
        { name: "Đang thực hiện", value: progressSteps.filter(p => p.trangThai === "Đang thực hiện").length },
        { name: "Chưa bắt đầu", value: progressSteps.filter(p => p.trangThai === "Chưa bắt đầu").length }
      ];

      // Supplier statistics by country
      const suppliers = await db.select().from(schema.nhaCungCap);
      const supplierCountryData = [
        { name: "Singapore", value: suppliers.filter(s => s.diaChi.includes("Singapore")).length },
        { name: "Tây Ban Nha", value: suppliers.filter(s => s.diaChi.includes("Tây Ban Nha") || s.diaChi.includes("Madrid")).length },
        { name: "CH Séc", value: suppliers.filter(s => s.diaChi.includes("CH Séc") || s.diaChi.includes("Praha")).length },
        { name: "Đức", value: suppliers.filter(s => s.diaChi.includes("Đức") || s.diaChi.includes("Berlin")).length },
        { name: "Áo", value: suppliers.filter(s => s.diaChi.includes("Áo") || s.diaChi.includes("Vienna")).length },
        { name: "Thụy Sĩ", value: suppliers.filter(s => s.diaChi.includes("Thụy Sĩ") || s.diaChi.includes("Zurich")).length }
      ...
      for (const countryData of supplierCountryData) {
        ...
        const countrySuppliers = suppliers.filter(s => {
          if (countryName === "Singapore") return s.diaChi.includes("Singapore");
          if (countryName === "Tây Ban Nha") return s.diaChi.includes("Tây Ban Nha") || s.diaChi.includes("Madrid");
          if (countryName === "CH Séc") return s.diaChi.includes("CH Séc") || s.diaChi.includes("Praha");
          if (countryName === "Đức") return s.diaChi.includes("Đức") || s.diaChi.includes("Berlin");
          if (countryName === "Áo") return s.diaChi.includes("Áo") || s.diaChi.includes("Vienna");
          if (countryName === "Thụy Sĩ") return s.diaChi.includes("Thụy Sĩ") || s.diaChi.includes("Zurich");
          return false;
        });
        
        // Count contracts for this country's suppliers
        const contractCount = contracts.filter(contract => 
          countrySuppliers.some(supplier => supplier.id === contract.nhaCungCapId)
        ).length;
  ...
  app.post("/api/loai-hop-dong", async (req, res) => {
    try {
      ...
      const items = await db.insert(schema.loaiHopDong).values(validatedData).returning();
  ...
  app.post("/api/can-bo", async (req, res) => {
    try {
      ...
      const items = await db.insert(schema.canBo).values(validatedData).returning();
      res.status(201).json(items[0]);
    } catch (error) {
  ...
  app.post("/api/nha-cung-cap", async (req, res) => {
    try {
      ...
      const items = await db.insert(schema.nhaCungCap).values(validatedData).returning();
      res.status(201).json(items[0]);
    } catch (error) {
  ...
  app.post("/api/chu-dau-tu", async (req, res) => {
    try {
      ...
      const items = await db.insert(schema.chuDauTu).values(validatedData).returning();
      res.status(201).json(items[0]);
    } catch (error) {
  ...
  app.post("/api/hop-dong", async (req, res) => {
    try {
      ...
      const items = await db.insert(schema.hopDong).values(validatedData).returning();
      res.status(201).json(items[0]);
    } catch (error) {
  ...
  app.post("/api/thanh-toan", async (req, res) => {
    try {
      ...
      const items = await db.insert(schema.thanhToan).values(validatedData).returning();
      res.status(201).json(items[0]);
    } catch (error) {
  ...
  app.post("/api/buoc-thuc-hien", async (req, res) => {
    try {
      ...
      const items = await db.insert(schema.buocThucHien).values(validatedData).returning();
      res.status(201).json(items[0]);
    } catch (error) {
  ...
  app.post("/api/trang-bi", async (req, res) => {
    try {
      ...
      const items = await db.insert(schema.trangBi).values(validatedData).returning();
      res.status(201).json(items[0]);
    } catch (error) {
  ...
  app.post("/api/dia-diem-thong-quan", async (req, res) => {
    try {
      ...
      const items = await db.insert(schema.diaDiemThongQuan).values(validatedData).returning();
      res.status(201).json(items[0]);
    } catch (error) {
  ...
  app.get("/api/tiep-nhan/:id", async (req, res) => {
    try {
      ...
      const items = await db.select().from(schema.tiepNhan).where(eq(schema.tiepNhan.id, id));
      if (items.length === 0) {
  ...
  app.get("/api/tiep-nhan/hop-dong/:hopDongId", async (req, res) => {
    try {
      ...
      const items = await db.select().from(schema.tiepNhan).where(eq(schema.tiepNhan.hopDongId, hopDongId));
      res.json(items);
    } catch (error) {
  ...
  app.post("/api/tiep-nhan", async (req, res) => {
    try {
      ...
      const items = await db.insert(schema.tiepNhan).values(validatedData).returning();
      res.status(201).json(items[0]);
    } catch (error) {
  ...
  app.put("/api/tiep-nhan/:id", async (req, res) => {
    try {
      ...
      const items = await db.update(schema.tiepNhan).set(validatedData).where(eq(schema.tiepNhan.id, id)).returning();
      if (items.length === 0) {
        return res.status(404).json({ error: "Không tìm thấy tiếp nhận" });
      }
      res.json(items[0]);
  ...
  app.delete("/api/tiep-nhan/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const items = await db.delete(schema.tiepNhan).where(eq(schema.tiepNhan.id, id)).returning();
      if (items.length === 0) {
        return res.status(404).json({ error: "Không tìm thấy tiếp nhận" });
      }
      res.status(204).send();
